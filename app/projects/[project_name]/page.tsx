...
export default function ProjectPage({ params }: { params: Promise<{ project_name: string }> }) {
    const { project_name } = use(params);
    const router = useRouter();

    const project = projects.find((item) => item.title === decodeURIComponent(project_name));

    if (!project) {
        return <div className="p-20 text-center text-slate-100">Project not found</div>;
    }
    ...
    return (
        <SmoothScroll>
            <div className="bg-background min-h-screen flex flex-col">
                ...
                <main className="flex-grow pt-32 px-8 lg:px-24">
                    <motion.div
                    ...
                    >
                        <button
                            onClick={() => router.back()}
                            className="flex items-center gap-2 text-slate-500 hover:text-primary transition-colors text-sm uppercase tracking-widest font-medium mb-8"
                        >
